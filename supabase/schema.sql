...
CREATE OR REPLACE FUNCTION create_order_with_items(p_user uuid, p_items jsonb, p_shipping jsonb DEFAULT '{}'::jsonb)
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    total := total + (qty * up);
    -- check stock
    IF NOT EXISTS (SELECT 1 FROM products WHERE id = (item->>'product_id')::uuid AND stock >= qty) THEN
      RAISE EXCEPTION 'out_of_stock for %', item->>'product_id';
    END IF;
  END LOOP;

  INSERT INTO orders (user_id, status, total, shipping) VALUES (p_user, 'pending', total, p_shipping) RETURNING id INTO o_id;
  ...
  FOR item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- decrement stock
    UPDATE products SET stock = stock - (item->>'quantity')::int WHERE id = (item->>'product_id')::uuid;
  END LOOP;
